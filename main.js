...
function update_board_view() {
    ...
    $('.number_cell').css('line-height', cell_side_length + 'px'); $('.number_cell').css('font-size', 0.6 * cell_side_length + 'px');
}
            ...
            if (board[i][j] != 0) {
                for (var k = 0; k < j; k++) {
                    if (board[i][k] == 0 && no_block_horizontal(i, k, j, board)) { 
                        ...
                        board[i][j] = 0;
                        //add score
                        score += board[i][k]; update_score(score); has_conflicted[i][k] = true; break;
                    } 
                }
            } 
        }
...
function move_right() {
    ...
    setTimeout('update_board_view()', 200); return true;
}
